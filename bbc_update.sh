...
/Users/crwelton/virtualenv/golfgenius3.10.10/bin/bbc-sync --results-directory='./results' uspfuw;
retVal=$?;

if [ $retVal -eq 0 ]; then
    echo "round data synced, regenerating website data...";
    /Users/crwelton/virtualenv/golfgenius3.10.10/bin/bbc-stats --results-directory='./results' --github-site='./website_root' --points-config-file='./website_root/_data/points_config.json' --blacklist-rounds-file='./website_root/_data/BLACKLISTED_ROUNDS'
    retVal=$?;
    if [ $retVal -eq 0 ]; then
        echo "Building website...";
        /Users/crwelton/.rubies/ruby-3.1.3/bin/bundle exec /Users/crwelton/.rubies/ruby-3.1.3/bin/jekyll build --source='./website_root' --destination='./docs'
        retVal=$?;
        if [ $retVal -eq 0 ]; then
